...
class ChatService {
            ...
            DatabaseFactory.dbQuery {
                ...
                val conversations =
                        Conversations.select {
                                ...
                                .orderBy(Conversations.lastMessageAt to SortOrder.DESC_NULLS_LAST)
                ...
                conversations.map { row ->
                    ...
                    val displayName =
                            if (firstMessage?.get(Messages.senderId) == userId) {
                                ...
                                val otherUser =
                                        Users.select { Users.id eq otherUserId }.singleOrNull()
                                otherUser
                                        ?.get(Users.email)
                                        ?.substringBefore("@")
    ...
    suspend fun getMessages(conversationId: String, userId: String): List<Message> =
            DatabaseFactory.dbQuery {
                ...
                val conversation =
                        Conversations.select { Conversations.id eq conversationId }.singleOrNull()
                ...
                Messages.update({
                    (Messages.conversationId eq conversationId) and (Messages.senderId neq userId)
                }) { it[isRead] = true }
        ...
        val identity =
                AnonymousIdentities.select {
                            (AnonymousIdentities.userId eq userId) and
                                    (AnonymousIdentities.conversationId eq conversationId)
                        }
                        .singleOrNull()
    ...
    private fun createAnonymousIdentity(userId: String, conversationId: String) {
        ...
        do {
            ...
            exists =
                    AnonymousIdentities.select {
                                (AnonymousIdentities.conversationId eq conversationId) and
                                        (AnonymousIdentities.anonymousName eq anonymousName)
                            }
                            .count() > 0
        } while (exists)
